...
1. Read `AGENTS.md`.
2. Read `.project-agent-workflow/docs/agent/spec-index.yaml`.
3. Select every route needed to cover the task without unrelated reads.
4. Union their `required` docs, add matching `conditional` docs, and read that set with `default_reads` before editing.
...
- Complete active work by archiving the active plan to `docs/plan/checked/` with the generated plan lifecycle scripts.
